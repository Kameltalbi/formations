...
export default function CheckoutIntl() {
  ...
  // Prix fixe pour l'international
  const unitPrice = 300
  const unitLabel = formatUSD(unitPrice)
  ...
  const validate = () => {
    ...
    if (!form.country) return "Pays requis."
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.paymentMethod === "card") {
        ...
        navigate(`/payment-intl?${params.toString()}`);
        return
      }

      // Pour MoneyGram, enregistrer directement
  ...
  if (isTN) {
    return (
      <div className="min-h-screen bg-light">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="text-center">
            <div className="text-blue-600 text-xl mb-4">🇹🇳 Page réservée à l'international</div>
            <p className="text-gray-600 mb-6">Cette page est destinée aux résidents hors Tunisie.</p>
            <Link 
              to="/checkout-tn"
              className="bg-primary text-white px-6 py-3 rounded-lg hover:bg-green-700 transition-colors"
            >
